#! /home/hervey/.virtualenv/misc/bin/python
import sys
import argparse
import os
import pathlib
...
if __name__ == "__main__":
  if len(sys.argv) == 1:
    ...
    parser.add_argument('-l', '--list', dest='func_list',
                        type=str, nargs=1, help='func list to be calculated')
        ...
        for func_str in func_list:
          ...
          if find_func:
            if find_cnt[func_str]['cnt'] == 1:
              ...
              if compare_data == 0:
                miscs.print_err(
                    data, "/", find_cnt[func_str]['base'], " is 0 with ", func_str)
              find_cnt[func_str]['datas'].append(compare_data)
              contents_output += func_str+": " + \
                  str(compare_data)+" ("+str(data)+")"+newline_symbol
    if image_output != '':
      """
      ...
      if use_pygal:
        import pygal
        line_chart = pygal.Line(show_legend=True, human_readable=True,
                                legend_at_bottom=True, x_title='denominator_list', y_title='data/base')
