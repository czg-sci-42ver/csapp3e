...
Also see /mnt/ubuntu/home/hervey/csapp3e/self_test/miscs_test/Guard_digit scripts.
...
[/mnt/ubuntu/home/hervey/csapp3e/self_test/miscs_test/Guard_digit]$ nopie_gcc Guard_digit_changed.c -std=c99 -mfpmath=sse -o Guard_digit_changed.o -pedantic -O0 -g
...
def parse_long_double(annotate_str, hex_num, bits, exp_size):
  ...
  frac = decimal.Decimal(
      int(my_bin[exp_size+1+1:bits], 2)) / decimal.Decimal(2**63)
...
split_hex = [perf_event_reg_str[i:i+4]
             for i in range(0, len(perf_event_reg_str), 4)]
print("origin hex: ", perf_event_reg_str)
print("each hex (rightest is LSB): ", split_hex)
print("CntMask: ", perf_event_reg_str[::-1][24:(31+1)][::-1])
print("16~23bit (rightest is 16bit): ",
      perf_event_reg_str[::-1][16:(23+1)][::-1])
"""
perf `l2_request_g2.group1` meaning

$ sudo su root -c "target_dir=/home/hervey;cd;echo ${target_dir};perf stat -a -e l2_request_g1.group2,l2_request_g1.rd_blk_l,l2_request_g1.rd_blk_x\
...
$ awk 'BEGIN { events=23;start_index=events-9;print start_index;begin_str_index=events+2;print begin_str_index;}{quote_str=$begin_str_index;printf("\"%s\":[ ",quote_str);for (i = start_index; i < start_index+5; ++i) {printf("%s, ",$i)};print "],"}' /mnt/ubuntu/home/hervey/csapp3e/debug/sample_num_prefetch.report 
14
25
"":[ 7678, 4680, 7700, 0, 0, ],
"":[ 7667, 4474, 7688, 0, 0, ],
...
def test_level_index_print(X: float):
  ...
  print(str(X)+": "+str(target_x)+"; X=" +
        str(level_index_num)+"; error: ", X-level_index_num)
...
print("Multiplicand: ", Multiplicand_signed,
      " and abs(Multiplicand): ", bin(abs(Multiplicand_signed)))
print("Multipler: ", Multiplier_signed,
      " and abs(Multiplicand): ", bin(Multiplier_signed))
print("mul result:", "{0:0{1}b}".format(arbitrary_len_signed_num(
    Multiplicand)*arbitrary_len_signed_num(Multiplier), result_bits))
