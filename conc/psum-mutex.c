...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nthreads; i++) {  // line:conc:psummutex:createloop1
    ...
    Pthread_create(&tid[i], NULL, sum_mutex,
                   &myid[i]);     // line:conc:psummutex:createloop3
...
void *sum_mutex(void *vargp) {
  ...
  /* Extract the thread ID */  // line:conc:psummutex:extractid
  long start = myid * nelems_per_thread;
  /* Start element index */  // line:conc:psummutex:getstart
  long end = start + nelems_per_thread;
  /* End element index */  // line:conc:psummutex:getend
  long i;
