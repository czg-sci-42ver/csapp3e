...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nthreads; i++) {  // line:conc:psumlocal:createloop1
    ...
    Pthread_create(&tid[i], NULL, sum_local,
                   &myid[i]);     // line:conc:psumlocal:createloop3
...
void *sum_local(void *vargp) {
  ...
  /* Extract the thread ID */  // line:conc:psumlocal:extractid
  long start = myid * nelems_per_thread;
  /* Start element index */  // line:conc:psumlocal:getstart
  long end = start + nelems_per_thread;
  /* End element index */  // line:conc:psumlocal:getend
  long i, sum = 0;
