...
void handler(int sig){
  ...
  Sigfillset(&mask_all);
  /*
  here not use `Waitpid` because sometimes process would be scheduled that CHILD exit before  
  */
  while (( pid = waitpid(-1, NULL, 0))>0) {
    ...
    */
    sprintf(buf, "%d", pid);
    /*
    sprintf() by `man sprintf`, it's undefined when ‘take place between ob‐jects that overlap’ ,although original code use.
    */
