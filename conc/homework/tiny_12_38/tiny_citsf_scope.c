...
static int cur_max_thread, old_max, max_items = 0;
...
void *check_thread(void *argp) {
  ...
  while (1) {
    ...
    if (sbuf_full(&sbuf)) {
      ...
      printf("upscale from %d\n", threads);
      for (i = threads; i < threads * 2; i++) /* Create worker threads */
        Pthread_create(&tid_set[i].tid, NULL, thread, &tid_set[i].mutex);
      if (cur_max_thread < threads) {
        cur_max_thread = threads;
      }
      threads *= 2;
    } else {
      /*
      not stuck in `sbuf_full(&sbuf)`
      */
      // Sio_puts("checked unfull\n");
    }
    if (sbuf_empty(&sbuf)) {
