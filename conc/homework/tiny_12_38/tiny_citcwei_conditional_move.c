...
#define USE_UPDATE_IF 1
/*
CONDITIONAL_MOVE doesn't influence a lot.
*/
#define CONDITIONAL_MOVE 0

sbuf_t sbuf;
static int threads;
struct thread_with_mutex {
  pthread_t tid;
  sem_t mutex;
};
...
void *check_thread(void *argp) {
  int buf_cnt, i, thread_double, thread_half;
  while (1) {
    buf_cnt = sbuf.rear - sbuf.front;
/*
not add unnecessary `if` although maybe decrease calculation.
*/
#if USE_UPDATE_IF
    if (update) {
#endif
      thread_double = threads * 2;
      thread_half = threads / 2;
#if USE_UPDATE_IF
    }
    update = 0;
#endif
    if (buf_cnt == sbuf.n) {
      ...
      for (i = threads; i < thread_double; i++) /* Create worker threads */
        Pthread_create(&tid_set[i].tid, NULL, thread, &tid_set[i].mutex);
#if !CONDITIONAL_MOVE
      threads = thread_double;
#endif
#if USE_UPDATE_IF
      update = 1;
#endif
...
#if CONDITIONAL_MOVE
    threads = buf_cnt == sbuf.n ? thread_double : threads;
#endif
    ...
    if (buf_cnt == 0) {
      ...
      for (i = thread_half; i < threads; i++) /* Create worker threads */
      {
        // printf("threads: %d\n", threads);
...
#if CONDITIONAL_MOVE
    threads = buf_cnt == 0 ? thread_half : threads;
#endif
  }
}
