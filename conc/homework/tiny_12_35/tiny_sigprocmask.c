...
void handler(int sig) {
  ...
  char buf[BUF_SIZE];
  /*
  1: here not use `Waitpid` with `while` because `Waitpid` must be -1 to exit
  while,then throw `exit(0)` to exit main thread which is unexpected. 2: since
  no need to calculate reap num, so `if` is just enough. (vs signal1.c &&
  signal2.c) ignore this: sometimes process would be scheduled that CHILD exit
  before
  */
