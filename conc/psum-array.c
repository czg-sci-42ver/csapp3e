...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nthreads; i++) {  // line:conc:psumarray:createloop1
    ...
    Pthread_create(&tid[i], NULL, sum_array,
                   &myid[i]);     // line:conc:psumarray:createloop3
...
void *sum_array(void *vargp) {
  ...
  /* Extract the thread ID */  // line:conc:psumarray:extractid
  long start = myid * nelems_per_thread;
  /* Start element index */  // line:conc:psumarray:getstart
  long end = start + nelems_per_thread;
  /* End element index */  // line:conc:psumarray:getend
  long i;
