...
__device__ int thread_sum(int *input, int n) 
{
	...
	// std::cout << blockIdx.x * blockDim.x + threadIdx.x << "; " << blockDim.x * gridDim.x << std::endl;
	// printf("%d; %d\n",blockIdx.x * blockDim.x + threadIdx.x,blockDim.x * gridDim.x);
	#endif
...
int main(){
	...
	Also see https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#concurrent-execution-between-host-and-device
